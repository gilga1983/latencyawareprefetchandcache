...
static void test_prefetch_delayed_hit() {
    ...
    assert(r.kind == AccessClass::PrefetchDelayed);
    assert(r.latency == 4);
    assert(sim.metrics().backend_fetches == 1);
}
...
static void test_completion_precedes_equal_time_demand() {
    ...
    assert(r.kind == AccessClass::ResidentHit);
    assert(r.latency == 0);
}
...
static void test_fill_time_controls_lru_and_pollution() {
    ...
    sim.advance_to(10);
    assert(sim.cache_contains(1));
    sim.advance_to(15);
    assert(!sim.cache_contains(1));
    assert(sim.cache_contains(2));
}
...
static void test_zero_latency_completes_immediately() {
    ...
    auto second = sim.demand({0, 41, 1});
    assert(second.kind == AccessClass::ResidentHit);
}
...
static void test_affine_backend() {
    ...
    auto r = sim.demand({0, 52, 9});
    assert(r.latency == 6);
}

static void test_time_reconstruction() {
    std::vector<Time> coarse{5, 5, 5, 6};
    auto t = spread_equal_timestamp_buckets(coarse, 1000);
    assert((t == std::vector<Time>{5250, 5500, 5750, 6500}));

    auto r = request_clock(4, 7, 3);
    assert((r == std::vector<Time>{3, 10, 17, 24}));
}

int main() {
    test_time_reconstruction();
    test_zero_latency_completes_immediately();
    test_affine_backend();
    test_latency_saving_curve();
    test_demand_coalescing();
    test_prefetch_delayed_hit();
    test_completion_precedes_equal_time_demand();
    test_fill_time_controls_lru_and_pollution();
    test_redundant_prefetch_does_not_touch_resident_lru();
    test_oversize_prefetch_is_suppressed_but_demand_fetches();
    std::cout << "all latency model tests passed\n";
    return 0;
}
